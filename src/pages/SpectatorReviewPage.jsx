...
export function SpectatorReviewPage() {
  ...
  async function handlePayment() {
    ...
    try {
      ...
      navigate(`/spectators/payment/checkout?draftId=${encodeURIComponent(state.draftId)}&orderId=${encodeURIComponent(orderId)}`);
    } catch (error) {
    ...
    <PageShell>
      <section className="site-page site-page--narrow">
        <div className="site-review-card site-spectator-review">
          ...
          <div className="site-inline-actions"><Button variant="ghost" onClick={() => navigate("/apply/spectator/consent")}>이전</Button><Button disabled={isPreparing} onClick={handlePayment}>{isPreparing ? "준비 중" : "결제 진행"}</Button></div>
